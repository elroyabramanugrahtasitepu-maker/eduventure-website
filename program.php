<?php 
// 1. Koneksi Database agar sinkron dengan admin
include 'config.php'; 
?>
<!DOCTYPE html>
<html lang="id">
  <head>
    ...
    <title>Program Unggulan | EDUVENTURE ABROAD</title>
    ...
    <style>
      ...
      .card-shadow:hover {
        ...
        box-shadow: 0 25px 50px -12px rgba(214, 0, 28, 0.2);
      ...
      .header-bg-custom::after {
        ...
        position: absolute;
        inset: 0;
        background: linear-gradient(to bottom, rgba(214, 0, 28, 0.2), var(--soft-bg));
        z-index: -1;
      }
    </style>
  </head>
  <body class="antialiased overflow-x-hidden">
      ...
      <div class="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div class="hidden md:flex flex-[2] justify-center items-center">
          <div class="bg-white/50 px-8 py-2 rounded-full border border-white/80 shadow-sm flex space-x-8 text-sm font-bold text-slate-600">
            ...
            <a href="program.php" class="text-gold transition-all">Program</a>
    ...
    <header class="pt-60 pb-32 relative overflow-hidden text-center header-bg-custom">
      <div class="container mx-auto px-6 relative z-10">
        <span class="inline-block px-4 py-1.5 bg-red-600 text-white rounded-full text-[10px] font-black uppercase tracking-[0.4em] mb-6">Our Services</span>
        <h1 class="text-5xl md:text-8xl font-extrabold text-slate-900 mb-6 tracking-tighter italic">Program <span class="text-gold">Eduventure</span></h1>
        <p class="text-slate-900 max-w-2xl mx-auto text-xl leading-relaxed font-extrabold">Perjalanan Anda menuju Bahasa Jerman & Prancis dimulai di sini</p>
      </div>
    </header>
    ...
    <section class="py-24 bg-white relative overflow-hidden">
      <div class="container mx-auto px-6 relative z-10">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
          <?php 
          $query_utama = mysqli_query($conn, "SELECT * FROM program ORDER BY id ASC LIMIT 3");
          $program_data = [];
          while($row_u = mysqli_fetch_assoc($query_utama)) { $program_data[] = $row_u; }

          for($i = 0; $i < 3; $i++): 
              $d = isset($program_data[$i]) ? $program_data[$i] : null;
          ?>
          <div class="p-10 rounded-[2.5rem] bg-white border border-red-50 shadow-sm card-shadow flex flex-col items-center text-center transform" data-aos="fade-up" data-aos-delay="<?= ($i + 1) * 100 ?>">
            <div class="w-24 h-24 rounded-full bg-red-50 flex items-center justify-center gap-2 mb-8 shadow-inner">
              <?php if($i == 0): ?>
                <img src="https://upload.wikimedia.org/wikipedia/en/b/ba/Flag_of_Germany.svg" alt="Germany" class="w-8 h-6 object-cover rounded shadow-sm" />
                <img src="https://upload.wikimedia.org/wikipedia/en/c/c3/Flag_of_France.svg" alt="France" class="w-8 h-6 object-cover rounded shadow-sm" />
              <?php elseif($i == 1): ?>
                <i class="fa-solid fa-file-signature text-3xl text-gold"></i>
              <?php else: ?>
                <i class="fa-solid fa-dragon text-3xl text-gold"></i>
              <?php endif; ?>
            </div>
            <span class="text-gold font-black uppercase text-[11px] tracking-[0.3em] mb-3">Program 0<?= $i + 1 ?></span>
            <h3 class="text-2xl font-extrabold text-slate-900 leading-tight mb-8 min-h-[4rem] flex items-center">
              <?= $d ? $d['judul'] : ($i == 0 ? "Kursus Bahasa <br /> Jerman & Prancis" : ($i == 1 ? "Pengurusan <br /> Dokumen & Visa" : "Pendampingan & <br /> Orientasi Awal")) ?>
            </h3>
            <button onclick="openDetail(<?= $d ? $d['id'] : ($i + 1) ?>)" class="mt-auto flex items-center gap-3 font-bold text-slate-900 hover:text-gold transition-all bg-red-50 px-6 py-3 rounded-full">Pelajari Detail <i class="fa-solid fa-arrow-right"></i></button>
          </div>
          <?php endfor; ?>
        </div>
      </div>
    ...
    <div id="detailModal" class="fixed inset-0 z-[500] hidden items-center justify-center px-4">
      <div class="absolute inset-0 bg-slate-900/80 backdrop-blur-md" onclick="closeDetail()"></div>
      <div id="detailModalContent" class="bg-white rounded-[2.5rem] p-8 md:p-12 w-full max-w-3xl relative z-10 shadow-2xl transform transition-all scale-95 opacity-0 overflow-y-auto max-h-[85vh]">
        <button onclick="closeDetail()" class="absolute top-6 right-6 w-10 h-10 flex items-center justify-center bg-slate-100 rounded-full text-slate-500 hover:bg-gold hover:text-white transition-all"><i class="fa-solid fa-xmark text-xl"></i></button>
        <div id="modalBody" class="text-slate-700"></div>
      </div>
    </div>
    ...
    <div id="mobileMenu" class="fixed inset-0 z-[200] invisible opacity-0 transition-all duration-500">
      ...
      <div id="menuContent" class="absolute right-0 top-0 w-full h-full bg-white shadow-2xl translate-x-full transition-transform duration-500 p-10 flex flex-col">
        ...
        <div class="flex flex-col gap-8 text-left">
          ...
          <a href="program.php" class="text-4xl font-black text-gold italic group">Program</a>
          ...
          <a href="https://maps.app.goo.gl/GVthAecFrcdNkA237?g_st=aw" target="_blank" class="text-4xl font-black text-gold bg-red-50 p-4 rounded-2xl border-2 border-red-200">Lokasi Maps <i class="fa-solid fa-location-dot animate-bounce"></i></a>
        </div>
    ...
    <footer class="bg-white border-t border-red-50 pt-24 pb-12">
      <div class="container mx-auto px-6 text-center md:text-left">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-12 mb-20 items-start">
          ...
          <div class="flex flex-col items-center justify-center"><h4 class="font-bold text-slate-900 mb-6 uppercase text-xs tracking-widest">Lokasi Kami</h4><a href="https://maps.app.goo.gl/GVthAecFrcdNkA237?g_st=aw" target="_blank" class="flex flex-col items-center gap-3 text-gold hover:scale-110 transition-all group"><div class="w-20 h-20 bg-red-50 rounded-3xl flex items-center justify-center shadow-lg group-hover:bg-red-600 group-hover:text-white transition-all"><i class="fa-solid fa-map-location-dot text-4xl"></i></div><span class="font-black italic text-sm uppercase">BUKA GOOGLE MAPS</span></a></div>
        ...
        <div class="pt-10 border-t border-red-50 text-center"><p class="text-slate-400 text-[10px] font-bold uppercase tracking-widest">© 2023 Eduventure Abroad - Gong Xi Fa Cai</p></div>
      </div>
    </footer>
    ...
    <script>
      ...
      const contents = {
        <?php
        $query_modal = mysqli_query($conn, "SELECT id, judul, deskripsi FROM program");
        while($rm = mysqli_fetch_assoc($query_modal)) {
            $desc_clean = str_replace(array("\r", "\n"), '', addslashes($rm['deskripsi']));
            echo $rm['id'] . ": `<h2 class='text-3xl font-black italic text-slate-900 mb-6'>".$rm['judul']."</h2><p class='text-lg leading-relaxed'>".$desc_clean."</p>`,";
        }
        ?>
      };

      function openDetail(id) {
        document.getElementById("modalBody").innerHTML = contents[id] || '<p class="p-8 text-center">Detail program belum tersedia.</p>';
        const modal = document.getElementById("detailModal");
        modal.classList.remove("hidden"); modal.classList.add("flex");
        setTimeout(() => { document.getElementById("detailModalContent").classList.remove("scale-95", "opacity-0"); document.getElementById("detailModalContent").classList.add("scale-100", "opacity-100"); }, 10);
        document.body.style.overflow = "hidden";
      }

      function closeDetail() {
        const content = document.getElementById("detailModalContent");
        content.classList.replace("scale-100", "scale-95"); content.classList.replace("opacity-100", "opacity-0");
        setTimeout(() => { document.getElementById("detailModal").classList.add("hidden"); document.body.style.overflow = "auto"; }, 300);
      }
